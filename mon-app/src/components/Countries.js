...
const Countries = () => {
    ...
    // Le useEffect se joue lorsque le composant est monté est mis en place
    useEffect(() => {
        axios.get("https://restcountries.com/v3.1/all").then((rest) => setData(rest.data))
    }, [])
    ...
    return (
        <div className='countries'>
            ...
            {selectedRadio && (
                <button onClick={() => setSelectedRadio("")}>Annuler la recharche</button>
            )}
            <ul>
                {
                    data
